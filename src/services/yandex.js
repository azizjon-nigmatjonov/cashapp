...
export const getAddressListYandex = (params, count = 0) =>
  new Promise((resolve, reject) => {
    axios({
      ...
      params: {
        ...
        type: "biz",
        lang: "ru_RU",
        apikey: placeSearchApiKeys[getRandomInt(placeSearchApiKeys.length - 1)],
        results: 5,
      ...
      .catch((err) => {
        ...
        return getAddressListYandex(params, count + 1);
      });
  });
...
export const getGeoCodeAddressList = (geocoder, params, count = 0) =>
  new Promise((resolve, reject) => {
    axios({
      ...
      params: {
        format: "json",
        apikey: pointSearchApiKeys[getRandomInt(pointSearchApiKeys.length - 1)],
        // apikey: 'asdasd',
        geocode: geocoder,
        sco: "latlong",
        lang: "ru-RU",
      ...
      .catch((err) => {
        if (count === 10) reject(err);
        return getGeoCodeAddressList(geocoder, params, count + 1);
      });
  });
